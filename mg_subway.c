...
void DrawCh(Character *m) {
    switch (m->action) {
        case RUN:
            ...
            break;
        case UP:
            SDL_BlitSurface(ch_jump, NULL, surface, &rech);
            break;
        default:
            SDL_BlitSurface(ch_down, NULL, surface, &rech);
            break;
    }
}
...
void Free(){
    ...
    SDL_FreeSurface(cha);
    SDL_FreeSurface(ch_jump);
    SDL_FreeSurface(ch_down);
    SDL_FreeSurface(coin);
    SDL_FreeSurface(ob_up);
    SDL_FreeSurface(ob_down);
    SDL_FreeSurface(ob_all);
    SDL_FreeSurface(ob_death);
}
